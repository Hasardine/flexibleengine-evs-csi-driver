...
const (
	cinderCSIClusterIDKey = "evs.csi.huaweicloud.com/cluster"
	DssIdKey = "dedicated_storage_id"
)

func (cs *controllerServer) CreateVolume(ctx context.Context, req *csi.CreateVolumeRequest) (*csi.CreateVolumeResponse, error) {
	klog.Infof("CreateVolume: called with args %+v", protosanitizer.StripSecrets(*req))
	...
	dssID := req.GetParameters()["dssId"]
	scsi := req.GetParameters()["scsi"]

	if len(volAvailability) == 0 {
		// Check from Topology
		if req.GetAccessibilityRequirements() != nil {
			volAvailability = getAZFromTopology(req.GetAccessibilityRequirements())
			klog.Infof("Get AZ By GetAccessibilityRequirements Availability Zone: %s", volAvailability)
		}
	}
	...
	if len(volumes) == 1 {
		...
		klog.Infof("Volume %s already exists in Availability Zone: %s of size %d GiB", volumes[0].ID, volumes[0].AvailabilityZone, volumes[0].Size)
		return getCreateVolumeResponse(&volumes[0], ignoreVolumeAZ, dssID, req.GetAccessibilityRequirements()), nil
	} else if len(volumes) > 1 {
		klog.Infof("found multiple existing volumes with selected name (%s) during create", volName)
	...
	if scsi != "" && (scsi == "true" || scsi == "false") {
		properties["hw:passthrough"] = scsi
	}
	...
	vol, err := cloud.CreateVolume(volName, volSizeGB, volType, volAvailability, snapshotID, sourcevolID, dssID, &properties)
	...
	err = cs.Cloud.WaitVolumeTargetStatus(vol.ID, targetStatus)
	if err != nil {
		klog.Errorf("Failed to WaitVolumeTargetStatus of volume %s: %v", vol.ID, err)
		return nil, status.Error(codes.Internal, fmt.Sprintf("[CreateVolume] Volume %s not in target state after create operation : %v", vol.ID, err))
	}
	klog.Infof("CreateVolume: Successfully created volume %s in Availability Zone: %s of size %d GiB", vol.ID, vol.AvailabilityZone, vol.Size)
	return getCreateVolumeResponse(vol, ignoreVolumeAZ, dssID, req.GetAccessibilityRequirements()), nil
}
...
func (cs *controllerServer) DeleteVolume(ctx context.Context, req *csi.DeleteVolumeRequest) (*csi.DeleteVolumeResponse, error) {
	...
	if err != nil {
		if cpoerrors.IsNotFound(err) {
			klog.Infof("Volume %s is already deleted.", volID)
	...
	klog.Infof("DeleteVolume: Successfully deleted volume %s", volID)
...
func (cs *controllerServer) ControllerUnpublishVolume(ctx context.Context, req *csi.ControllerUnpublishVolumeRequest) (*csi.ControllerUnpublishVolumeResponse, error) {
	...
	if err != nil {
		if cpoerrors.IsNotFound(err) {
			klog.Infof("ControllerUnpublishVolume assuming volume %s is detached, because node %s does not exist", volumeID, instanceID)
	...
	klog.Infof("ControllerUnpublishVolume %s on %s", volumeID, instanceID)
...
func getCreateVolumeResponse(vol *volumes.Volume, ignoreVolumeAZ bool, dssID string, accessibleTopologyReq *csi.TopologyRequirement) *csi.CreateVolumeResponse {
