...
func nodePublishEphemeral(req *csi.NodePublishVolumeRequest, ns *nodeServer) (*csi.NodePublishVolumeResponse, error) {
	klog.Infof("nodePublishEphemeral: called with args %+v", protosanitizer.StripSecrets(*req))
	var size int
	var err error

	volID := req.GetVolumeId()
	volName := fmt.Sprintf("ephemeral-%s", volID)
	properties := map[string]string{"evs.csi.huaweicloud.com/cluster": ns.Driver.cluster}
	...
	if ok && strings.HasSuffix(capacity, "Gi") {
		...
		if err != nil {
			klog.Infof("Unable to parse capacity: %v", err)
	...
	dssID := req.VolumeContext[DssIdKey]
	evol, err := ns.Cloud.CreateVolume(volName, size, volumeType, volAvailability, "", "", dssID, &properties)

	if err != nil {
		klog.Infof("Failed to Create Ephemeral Volume: %v", err)
	...
	devicePath, err := ns.getDevicePathV2(evol.ID)
	if err != nil {
	...
	if notMnt {
		...
		klog.Infof("Ephemeral notMnt, set default fstype is ext4")
		fsType := "ext4"
		var options []string
		if mnt := volumeCapability.GetMount(); mnt != nil {
			if mnt.FsType != "" {
...
func nodePublishVolumeForBlock(req *csi.NodePublishVolumeRequest, ns *nodeServer, mountOptions []string) (*csi.NodePublishVolumeResponse, error) {
	...
	source, err := ns.getDevicePathV2(volumeID)
	if err != nil {
		return nil, status.Error(codes.Internal, fmt.Sprintf("Unable to find Device path for volume: %v", err))
	}
...
func nodeUnpublishEphemeral(req *csi.NodeUnpublishVolumeRequest, ns *nodeServer, vol *volumes.Volume) (*csi.NodeUnpublishVolumeResponse, error) {
	...
	if err != nil {
		klog.Infof("Failed to DetachVolume: %v", err)
		return nil, status.Error(codes.Internal, err.Error())
	}

	err = ns.Cloud.WaitDiskDetached(instanceID, volumeID)
	if err != nil {
		klog.Infof("Failed to WaitDiskDetached: %v", err)
		return nil, status.Error(codes.Internal, err.Error())
	}

	err = ns.Cloud.DeleteVolume(volumeID)
	if err != nil {
		klog.Infof("Failed to DeleteVolume: %v", err)
		return nil, status.Error(codes.Internal, err.Error())
	}
...
func (ns *nodeServer) NodeStageVolume(ctx context.Context, req *csi.NodeStageVolumeRequest) (*csi.NodeStageVolumeResponse, error) {
	...
	m := ns.Mount
	// Do not trust the path provided by cinder, get the real path on node
	devicePath, err := ns.getDevicePathV2(volumeID)
	if err != nil {
		return nil, status.Error(codes.Internal, fmt.Sprintf("Unable to find Device path for volume: %v", err))
	}
...
func getDevicePath(volumeID string, m mount.IMount) (string, error) {
	klog.Infof("getDevicePath: called with args volumeID: %s", volumeID)
...
func (ns *nodeServer) getDevicePathV2(volumeID string) (string, error) {
	klog.Infof("getDevicePathV2: called with args volumeID: %s", volumeID)
	vol, err := ns.Cloud.GetVolume(volumeID)
	if err != nil {
		if cpoerrors.IsNotFound(err) {
			return "", status.Errorf(codes.NotFound, "[getDevicePathV2] Volume %s not found", volumeID)
		}
		return "", status.Error(codes.Internal, fmt.Sprintf("[getDevicePathV2] get volume failed with error %v", err))
	}
	klog.Infof("[getDevicePathV2] volumeID: %s, devicePath: %s", volumeID, vol.Attachments[0].Device)
