...
import (
	...
	volumeCreate "github.com/gophercloud/gophercloud/openstack/blockstorage/extensions/schedulerhints"
...
func (os *OpenStack) CreateVolume(name string, size int, vtype, availability string, snapshotID string, sourcevolID string, dssID string, tags *map[string]string) (*volumes.Volume, error) {

	volumeCreateReq := &volumes.CreateOpts{
		Name:             name,
		Size:             size,
		VolumeType:       vtype,
	...
	ext := &volumeCreate.CreateOptsExt{
		VolumeCreateOptsBuilder: volumeCreateReq,
	}

	schedulerHintMap := make(map[string]interface{})
	if dssID != "" {
		schedulerHintMap[DssIdKey] = dssID
	}
	schedulerHintReq := &volumeCreate.SchedulerHints{
		AdditionalProperties: schedulerHintMap,
	}
	ext.SchedulerHints = schedulerHintReq

	if tags != nil {
		volumeCreateReq.Metadata = *tags
	}
	volumeCreateReq.Metadata["create_for_volume_id"] = "true"

	vol, err := volumes.Create(os.blockstorage, ext).Extract()
	if err != nil {
		return nil, err
	}
...
func (os *OpenStack) WaitVolumeTargetStatus(volumeID string, tStatus []string) error {
	...
	waitErr := wait.ExponentialBackoff(backoff, func() (bool, error) {
		...
		klog.Infof("WaitVolumeTargetStatus: %s, status: %s ", volumeID, vol.Status)
...
func (os *OpenStack) DetachVolume(instanceID, volumeID string) error {
	...
	if volume.Status == VolumeAvailableStatus {
		klog.Infof("volume: %s has been detached from compute: %s ", volume.ID, instanceID)
		return nil
	}
	...
	for _, att := range volume.Attachments {
		if att.ServerID == instanceID {
			...
			klog.Infof("Successfully detached volume: %s from compute: %s", volume.ID, instanceID)
			return nil
		}
	}
